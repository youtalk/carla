...
```bash
export CARLA_UNREAL_ENGINE_PATH=/home/youtalk/src/UnrealEngine
UE=/home/youtalk/src/UnrealEngine/Engine/Binaries/Linux
PROJ=/home/youtalk/src/carla/Unreal/CarlaUnreal/CarlaUnreal.uproject
MODE=list-native TARGET_MAP=/Game/Carla/Maps/Town04_Opt DISPLAY=:0 \
  timeout 600 "$UE/UnrealEditor" "$PROJ" -run=pythonscript \
  -script="/home/youtalk/src/carla/Util/Tools/author_map_sky.py" \
...
Then run a luminance probe against the running simulation — e.g.
`cam_luminance_probe.py` (in the `carla-autoware-extension` repo), which
spawns a vehicle + RGB sensor, grabs a settled frame, and reports:
