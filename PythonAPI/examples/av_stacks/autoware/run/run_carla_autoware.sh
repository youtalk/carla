...
Usage: $(basename "$0") --mode classical|e2e [options]
  ...
  --goal "X,Y,YAW"       classical: drive to this goal after startup. CARLA
                         ...
                         (x_map = OX + x, y_map = OY - y, yaw_map = -yaw, with
                         OX,OY from --map-origin) automatically.
  --map-origin "X,Y,Z"   map-frame position of the CARLA world origin, metres
                         (default 0,0,0 = Local-projector maps such as Town10HD).
...
log "mode=$MODE town=$TOWN sim-rmw=$RMW ($RMW_IMPL) stack=$STACK domain-id=$DOMAIN_ID log-dir=$LOG_DIR map-origin=$MAP_ORIGIN"
...
# wheelbase, 1.425 m). The CARLA->map conversion takes --map-origin, so
# MGRS-projector maps gate correctly. Gate 2 listens to /diagnostics for
# distortion-corrector errors (a mismatched per-point time_stamp silently
# disables the corrector and NDT then degrades at speed).
...
# argv: <carla-host> <carla-rpc-port> <map-origin "X,Y,Z">; stdin: one `ros2
# topic echo --once /localization/kinematic_state` dump. Exit 0 pass, 1
# diverged, 2 no data yet.
import math
import sys
...
ORIGIN_X, ORIGIN_Y, _ = (float(v) for v in sys.argv[3].split(","))  # --map-origin (0,0,0 for Local maps)
...
gt_x, gt_y, gt_yaw = ORIGIN_X + tf.location.x, ORIGIN_Y - tf.location.y, -math.radians(tf.rotation.yaw)
...
run_engage_gates() {
    ...
    for attempt in $(seq 1 6); do
        ...
        rc=0
        printf '%s\n' "$belief" | "$CARLA_PY" "$GATE1_PY" "$CARLA_HOST" "$RPC_PORT" "$MAP_ORIGIN" | tee -a "$LOG_DIR/automation.log" || rc=$?
        if [[ $rc -eq 0 ]]; then
            gate1_ok=true
            break
        fi
...
if [[ "$MODE" == "classical" ]] && ! $NO_AUTO; then
    if wait_for_autoware_api; then
        ...
        if ! $GATES_OK; then
            ...
            # CARLA -> Autoware map frame: x_map = OX + x, y_map = OY - y, yaw_map = -yaw,
            # with (OX, OY) from --map-origin (0,0 for Local-projector maps).
            read -r GX GY GQZ GQW <<<"$(python3 -c "
import math
x, y, yaw = (float(v) for v in '$GOAL'.split(','))
ox, oy, _ = (float(v) for v in '$MAP_ORIGIN'.split(','))
r = math.radians(-yaw)
print(f'{ox + x:.3f} {oy - y:.3f} {math.sin(r / 2.0):.6f} {math.cos(r / 2.0):.6f}')
")"
