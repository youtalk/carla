...
class TestROS2(SyncSmokeTest):
    ...
    def test_ros2_hero_vehicle_topics_lifecycle(self):
        ...
        odometry, vehicle_status and latched vehicle_info publishers, and every
        tick publishes odometry/status from CarlaEngine::PublishROS2VehicleState.
        Destroying the hero and spawning a new one with the same ros_name
        re-creates the same topics, so this exercises the DDS writer cleanup in
