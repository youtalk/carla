...
* Added `mgrs_offset_x/y/z` blueprint attributes (Float, metres, Autoware map frame) to `sensor.other.autoware_gnss`, so a level that ships no `UMgrsDataAsset` — any imported digital twin, and every map a user cannot re-save — can still publish its GNSS pose in the MGRS map frame that an Autoware lanelet2/PCD map expects, instead of an unoffset one. The level's `UMgrsDataAsset` remains the primary source: the attributes are read only when the level provides none, and a non-zero fallback set alongside a present asset is logged and ignored rather than blended in. The wire values are parsed as double, since an offset of ~1e5 m would resolve to ~8 mm as float32. `autoware_demo.py` gains `--mgrs_offset` and `run_carla_autoware.sh` forwards `--map-origin` to it.
